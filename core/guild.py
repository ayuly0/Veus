...
class Guild:
	...
	def change_guild_icon(self, icon: str) -> list:
		payload = {"icon": icon}
	...
	def delete_channel_by_id(self, channel_id: int) -> list:
		return self.rq.delete([f"channels/{channel_id}"])

	def delete_all_channels(self) -> Union[list, None]:
	...
	def delete_channels(self, channel_ids: list) -> Union[list, None]:
	...
	def delete_all_emojis(self) -> list:
		emojis, _ = self.get_all_emojis()
		emoji_paths = [self._route(f"emojis/{emoji['id']}") for emoji in emojis]
		return self.rq.delete(emoji_paths)

	def delete_role_by_id(self, role_id: int) -> list:
		return self.rq.post(f"roles/{role_id}")

	def delete_all_roles(self) -> Union[list, None]:
		roles, _ = self.get_all_roles()
		role_paths = []
		for role in roles:
	...
	def ban_user_by_id(self, user_id: int) -> tuple:
		payload = {"delete_message_days": 0, "reason": "by Veus"}
		return self.rq.put(f"bans/{user_id}", [payload])[0]

	def ban_all_users(self) -> list:
		payload = {"delete_message_days": 0, "reason": "by Veus"}
		users = self.get_all_users()
