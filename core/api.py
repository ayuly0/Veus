...
class API:
	...
	async def aget(self,
	               api_path: str,
	               session,
	               proxy: Union[str, None] = None) -> tuple:
		url = self._route(api_path)
		try:
			async with async_timeout.timeout(360):
				async with session.get(url, headers=self._header,
	...
	async def apost(self,
		...
		while True:
			try:
				...
				if all("10054" not in str(ex), "10053" not in str(ex),
				       "Cannot connect" not in str(ex),
				       "Expecting value: line 1" not in str(ex), "503"
				       not in str(ex), "Server disconnected" not in str(ex)):
					self._logger("error", ex)
					return {}

	async def apatch(self,
	                 api_path: str,
	                 payload: dict,
	                 session,
	                 proxy: str = "") -> tuple:
